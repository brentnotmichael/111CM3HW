...
public class HomeworkM3Driver {
	...
	public static boolean containsDuplicates(BagInterface<String> wordBag) {
			...
			while (!wordBag.isEmpty()) {
				String tempItem = wordBag.remove();
				tempBag.add(tempItem);
				if (wordBag.contains(tempItem)) dupesFlag = true;
			}
	...
	public static ListInterface<String> createListInterfaceContainingChar(ListInterface<String> wordList, char key) {
		if (wordList.isEmpty()) return new AList<String>();	// short circuit for empty wordList
		ListInterface<String> returnList = new AList<String>();
		for (int i = 1; i <= wordList.getLength(); i++) {	// get word
			String word = wordList.getEntry(i);
			for (int c = 0; c < word.length(); c++) {		// get char of word
				if (word.charAt(c) == key) {				// compare char at index to key
					if (!returnList.contains(word)) returnList.add(word);	// no duplicates in return list
				}
			}
		}
		return returnList;
	...
	public static int lastPosition(List<String> wordList, String targetWord) {
		int returnPos = -1;	// if word isn't in list will return -1 as index
		int len = wordList.size();	// single call to .size() for efficiency
		for (int w = 0; w < len; w++ ) {
			if (wordList.get(w).equals(targetWord)) returnPos = w;
		}
		return returnPos;
	}

	public static boolean equivalentLists(ListInterface<Integer> numberListInterface, List<Integer> numberList) {
		if (numberListInterface.isEmpty() && numberList.isEmpty()) return true;	// both empty, both equiv
		if (numberListInterface.getLength() != numberList.size()) return false; // can't be equiv if not same length
		int len = numberListInterface.getLength();
		for (int i = 1; i <= len; i++) {
			if (numberListInterface.getEntry(i) != numberList.get(i-1)) return false;
		}
		return true;	// lists eqiv
	}

	public static void prioritizeMaximumValue(List<Integer> numberList) {
		if (numberList.size() > 1) { 	// skip this if List has 0 or 1 items in it
			int highestInt = Integer.MIN_VALUE;	// lowest int possible!
			for (Integer n : numberList) {	// get the highest integer first
				if (n > highestInt) highestInt = n;
			}
			for (int i = 0; i < numberList.size(); i++) {	// then put highest at 0 pos, remove original pos (shifting all others)
				if (numberList.get(i) == highestInt) numberList.add(0, numberList.remove(i));
				i++;
			}
		}
	}
